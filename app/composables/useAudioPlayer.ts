...
export const useAudioPlayer = (_options: TOptions) => {
  if (!_options) {
  ...
  const { onSetPlaylist, onResetPlaylist, playlist, playlistOptions } = useAudioPlayerList();
  ...
  const init = () => {
    ...
    const {
      ...
      progressColor = 'linear-gradient(90deg, #06b6d4, #8b5cf6, #ec4899)',
      barWidth = 2,
      barRadius = 3,
      barGap = 2,
      barHeight = 0.9,
